...
import { getbrandproduct, getproduct, searchproduct } from "@/service";
...
export function ListBrandSearch({ product }: IListProductProps) {
  ...
  const handlechangepagi = async (page: any) => {
    let id = router.query.brand[1];

    let limit = product.limit;
    let offset = (+page - 1) * limit;
    let res = await getbrandproduct({
      limit: limit + offset,
      id: id,
      page: +page,
      offset,
  ...
  return (
    <Box mt={"20px"}>
      <Container maxWidth={"xl"} sx={{ width: "1300px" }}>
        <Header name={router && router.query.brand && router.query?.brand[0]} />
        <Stack>
          <Typography variant='h5' textAlign={"center"}>
            Thương Hiệu “
            {router && router.query.brand && router.query?.brand[0]}” <br></br>
            <span style={{ color: "grey", fontSize: "14px" }}>
              Tìm thấy {product.count} kết quả phù hợp với Thương hiệu “
              {router && router.query.brand && router.query?.brand[0]}”
            </span>
          </Typography>
