...
import anh from "../../image/lấy-tóc.jpg";
import Slider from "react-slick";
import Link from "next/link";
import { useRouter } from "next/router";

export interface ICSTVDDProps {
  rolecstvdd: [
    {
      id: number;
      Rolename: string;
...
export function CSTVDD({ rolecstvdd, productcstvdd }: ICSTVDDProps) {
  ...
  const handlerole = (id: number) => {
    history.push(`/Chăm sóc tóc và da đầu/5/${id}`);
  };
  return (
    <Box
      width='100%'
      ...
      <Stack direction={"row"} justifyContent={"space-between"}>
        ...
        <Box width={846} overflow={"hidden"} position={"relative"}>
          ...
          <Box display={"flex"} flexWrap={"wrap"}>
            {productcstvdd &&
              productcstvdd.length &&
              productcstvdd.map((item, index) => {
          ...
          <Link
            href={`/${productcstvdd[0].Category.categoryname}/${productcstvdd[0].category_id}`}>
            <Box
              mt={"20px"}
              display={"flex"}
              alignItems={"center"}
