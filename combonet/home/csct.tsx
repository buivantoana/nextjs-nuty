...
import anh from "../../image/quế-hồi.jpg";
import Slider from "react-slick";
import Link from "next/link";
import { useRouter } from "next/router";

export interface ICSCTProps {
  rolecsct: [
    {
      id: number;
      Rolename: string;
...
export function CSCT({ rolecsct, productcsct }: ICSCTProps) {
  ...
  const handlerole = (id: number) => {
    history.push(`/Chăm sóc cơ thể /3/${id}`);
  };
  return (
    <Box
      width='100%'
      ...
      <Stack direction={"row"} justifyContent={"space-between"}>
        ...
        <Box width={846} overflow={"hidden"} position={"relative"}>
          ...
          <Link
            href={`/${productcsct[0].Category.categoryname}/${productcsct[0].category_id}`}>
            <Box
              mt={"20px"}
              display={"flex"}
              alignItems={"center"}
